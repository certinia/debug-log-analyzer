...
- **Present tense.** "reduce", "show", "flag" — never "reduced", "showed".
...
- **Sections in this order:** Added, Changed, Removed, Fixed.
...
The emoji and label carry the verb, so the sentence says what the user gets, not what was done to
the code. Pick an emoji no other entry in the section uses.

The root `CHANGELOG.md` is the source. The `lana/` copy is generated by the build — never edit it.

**The file outranks this skill on style.** Read the released sections first, and match their voice:
a changelog that switches voice mid-file reads worse than one in the wrong voice. Length and jargon
are not style — those rules hold everywhere.
