...
![Timeline view screenshot showing a color-coded flame chart of Salesforce event types such as DB, Method, and SOQL, visualizing code execution duration and performance](https://raw.githubusercontent.com/certinia/debug-log-analyzer/main/lana/assets/1_22/timeline.png)

The timeline stays fast on large logs. It includes text labels on frames, fast zoom/pan operations, and a natural time axis scaling.
...
Selecting a frame also fills the [inspector](./inspector.md), so you can read its governor metrics, call stack and scoped call tree beside the chart.
...
Log issue markers (truncation, errors, exceptions, etc.) can be selected and navigated. **Exceptions** draw as a thin red hairline at the moment each was thrown; a dense cluster collapses into a single "N exceptions" marker. Method tooltips also show a **Throws** count for exceptions thrown anywhere in that method's subtree.
...
<img
  src="https://raw.githubusercontent.com/certinia/debug-log-analyzer/main/lana/assets/1_22/timeline-tooltip.png"
...
- **Throws** - Number of exceptions thrown within that event's subtree, when any occurred
...
<img
  src="https://raw.githubusercontent.com/certinia/debug-log-analyzer/main/lana/assets/1_22/timeline-minimap.png"
  alt="The Timeline minimap: a density skyline of the whole log, with the viewport lens over the part the timeline is showing."
  style={{
    width: '100%',
    height: 'auto',
    maxWidth: '100%',
...
<img
  src="https://raw.githubusercontent.com/certinia/debug-log-analyzer/main/lana/assets/1_22/timeline-gov-strip.png"
...
#### Where the log reports no limits

A percentage needs a limit, and the log is the only source of one. Where it reports none, each metric is scaled to its own highest point instead, so the shape of usage over time still reads. The band, the 100% line, the breach shading and the traffic-light colours all mark a distance from a limit, so they come off; the collapsed strip shades by level in a single neutral grey, and the tooltip reads `770 of 1,240` rather than `770 / 1,240`.
