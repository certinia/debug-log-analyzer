...
The Database tab's detail side bar (see [Database → Detail side bar](./features/database.md#detail-side-bar)) remembers where you dock it and how big it is:

- `lana.sidePanel.position` – where it docks: `right` (default), `left` or `bottom`.
- `lana.sidePanel.size` – its width (or height when docked to the bottom), in pixels.

Both update as you drag or re-dock the panel, so you rarely need to edit them by hand.
