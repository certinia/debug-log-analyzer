...
`lana.callTree.columnView` sets the column view the Call Tree and Analysis tables open with: `General` (default), `Time`, `Governor Limits`, `Database` or `Memory`. Switching views from the **Columns** menu updates this setting, so the tables reopen the way you left them. See [Call Tree → Column Views](./features/calltree.mdx#column-views).
