...
export type TimelineKey = 'method' | 'systemMethod' | 'codeUnit' | 'dml' | 'soql' | 'flow' | 'workflow';
 ...
 */
 export abstract class LogLine {
  ...
  declarative: boolean = false;	// is this log entry generated by a declarative process?
  isExit: boolean = false;		// is a method exit line?
  isValid: boolean = true;		// is this a valid method which will have children?
  ...
  hasValidSymbols: boolean = false;	// can we open source for this node?
 ...
 */
 export class TimedNode extends LogLine {
  ...
  duration: number | null = null;		// the time spent in the node
  ...
  containsDml: number = 0;			// the number of DML_BEGIN decendants in a node 
  containsSoql: number = 0;			// the number of SOQL_EXECUTE_BEGIN decendants in a node
  containsThrown: number = 0;			// the number of EXCEPTION_THROWN decendants in a node
