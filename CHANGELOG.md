...
- 🧭 **Inspector**: select a timeline frame, a table row or a statement to see its details, governor usage, call stack and subtree - or select nothing for a whole-log overview. Dock it left, right or bottom, drag a section header to reorder it or press `Alt+Up` / `Alt+Down`, right-click a header to choose which sections show, and drag a divider to resize. Each tab keeps its own arrangement. ([#113] [#373] [#63])
