...
- 🧭 **Inspector**: select anything — a timeline frame, a call tree or analysis row, a SOQL/DML/SOSL statement — and inspect it without leaving the tab you're on. ([#113])
  - **A selection** shows its details and governor metrics as a share of what the transaction consumed, the call stack that led to it, and its own subtree in **Time Order**, **Aggregated** or **Bottom-Up**. Click a frame in the call stack to walk up it — the details and subtree follow, and the stack stays anchored to what you selected. On the Timeline it also splits the self time under the selection by the namespace whose code ran it.
...
- 📏 **Governor limits come only from the log**: a limit is no longer assumed where the log reports none, so a figure is never measured against Salesforce's synchronous maximum for a transaction that had a different one. Without a reported limit the gauges read as levels, the Timeline strip and the governor trends scale each metric to its own peak, and the governor cost columns read `—`.
- 📊 **Governor columns in the Call Tree and Analysis** now fill against what the whole transaction consumed, like the time columns beside them, so the path responsible for a metric stands out instead of drawing a 3% sliver. **Gov Avg %** and **Gov Peak %** still measure headroom against the limits, and each cell's tooltip still names the limit.
